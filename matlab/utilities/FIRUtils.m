classdef FIRUtils
    ...
    methods (Static)
        ...
        function [fdata] = plotXYZ(b, F, A, w, h, data, freq)
        ...
        function [fdata, b, F, A, h, w] = runLowPassXYZ(data, cutoff, freq, ntaps)
            F = [0, cutoff*2/freq,  cutoff*2/freq,  1];      % Frequency breakpoints
            A = [1, 1,              0,              0];          % Magnitude breakpoints
            [b, h, w] = FIRUtils.getFilter(ntaps, F, A);
            fdata = FIRUtils.plotXYZ(b, F, A, w, h, data, freq);
        end
        
        function [fdata, b, F, A, h, w] = runHighPassXYZ(data, cutoff, freq, ntaps)
            F = [0, cutoff*2/freq,  cutoff*2/freq,  1];      % Frequency breakpoints
            A = [0, 0,              1,              1];          % Magnitude breakpoints
            [b, h, w] = FIRUtils.getFilter(ntaps, F, A);
            fdata = FIRUtils.plotXYZ(b, F, A, w, h, data, freq); % return filtered
        end
    end    
end
